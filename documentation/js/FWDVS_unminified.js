	...
	var FWDVS = function(props_obj){
		...
		self.onInitlalizeScrollHandler = function(){
			...
			if(self.main_do.getRect().top >= -self.stageHeight && self.main_do.getRect().top < self.viewportSize.h){
				...
				self.thumbnailManager_do.ssImages();
			}
		};
...
(function (window){
	...
	var FWDVSClassicVerticalThumbnailsManager = function(data, parent){
		...
		this.getThumbnail =  function(cats, id){
			...
			props_obj.isDisabled_bl = this.sourcePlaylist_ar[id]._bl;
		...
		this.onImageLoadHandler = function(e){
			...
			curThumbnail_do.originalWidth = self.image_img.naturalWidth;
			curThumbnail_do.originalHeight = self.image_img.naturalHeight;
			self.loadedId ++;
	...
	var FWDVSData = function(props, parent){
		...
		this.parseProperties = function(){
			...
			test = self.rightClickContextMenu_str == "developer" 
				   || self.rightClickContextMenu_str == ""
		...
		this.finalizePlaylist = function(){
			...
			if(self.randomizeSlider_bl) self.playlist_ar.playlistItems = FWDVSUtils.randomizeArray(self.playlist_ar.playlistItems);
...
var FWDVSSimpleSizeButton = function(
	...
	var FWDVSThumbnail = function(props_obj){
		...
		this.addImage = function(image){
			...
			self.imageOriginalW = image.naturalWidth;
			self.imageOriginalH = image.naturalHeight;
		...
		this.resizeAndPosition = function(overwrite){	
			this.borderSize = 20;
			this.border_do.setX(this.borderSize);
			this.border_do.setY(this.borderSize);
