...
var FWDEVPCC = function(
		...
		this.loadSubtitle = function(){
			...
			itemsInfoRequest = new chrome.cast.media.EditTracksInfoRequest([main.ccSS]);
			media.editTracksInfo(itemsInfoRequest, function(e){},function(e){console.log(e);});
		}
		...
		this.loadMedia = function(autoplay){
			...
			if(subData){
				...
				for(var i=0; i<subData.length; i++){
					var item = new chrome.cast.media.Track(i + 1, chrome.cast.media.TrackType.TEXT);
					item.itemContentId = FWDEVPUtils.getValidSource(subData[i]['subtitlePath']);
					item.itemContentType = 'text/vtt';
					item.subtype = chrome.cast.media.TextTrackType.SUBTITLES;
					item.name = subData[i]['subtileLabel'];
					item.customData = null;
					items[i] = item;
				}
				
				item = new chrome.cast.media.Track(0, chrome.cast.media.TrackType.TEXT);
				item.itemContentId = FWDEVPUtils.getValidSource('content/subtitles/empty.vtt');
				item.subtype = chrome.cast.media.TextTrackType.SUBTITLES;
				item.name = '';
				item.customData = null;
				item.itemContentType = 'text/vtt';
			...
			cast.framework.CastContext.getInstance().getCurrentSession().loadMedia(request).then(
				function() {
					if(subData){
						...
						itemsInfoRequest = new chrome.cast.media.EditTracksInfoRequest([main.ccSS]);
						media.editTracksInfo(itemsInfoRequest, function(e){
